      ...
      <section className="tables-section" id="tables" aria-labelledby="tables-title"><div className="section-heading"><div><span className="eyebrow">GOOD COMPANY. GREAT COMPETITION.</span><h2 id="tables-title">Pull up a chair.</h2></div><p>Bring your friends, or add a bot.<br />The first move is yours.</p></div>
        <div className="home-grid">
          ...
          <section className="card join-table-card" aria-labelledby="join-title"><div className="form-title"><span className="form-icon" aria-hidden="true">↗</span><div><h3 id="join-title">Already invited?</h3><p>Good. They saved you a seat.</p></div></div>
            ...
            <div className="room-list">{rooms.length === 0 && <div className="empty-tables"><span aria-hidden="true">▧</span><strong>A quiet moment before the rivalry.</strong><p>Start a table and get the evening going.</p></div>}{rooms.map(room => { const lobby = room.phase === 'lobby', full = room.playerCount >= room.maxPlayers, spectatorFull = room.spectatorReservedCount >= room.spectatorLimit, canWatch = room.spectatorOpen && !spectatorFull; return <button type="button" key={room.id} className="room-row" disabled={!canJoin || (lobby ? full : !canWatch)} onClick={() => joinRoom(room.id)}><span className="room-name">{room.name}</span><span className="room-meta">{room.ruleWorldName} · {room.playerCount}/{room.maxPlayers} · {lobby ? (full ? 'full' : 'open') : canWatch ? 'watch game' : spectatorFull ? 'spectators full' : 'spectators closed'}{room.spectatorCount > 0 ? ` · ${room.spectatorCount} watching` : ''}</span><span className="room-code">{room.id}</span></button>; })}</div>
          </section>
        </div>
      </section>
