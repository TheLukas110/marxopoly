 ...
 */
export type ColorGroup = 'clay' | 'sky' | 'rose' | 'amber' | 'crimson' | 'gold' | 'forest' | 'navy';
export type OwnableGroup = ColorGroup | 'depot' | 'works';
export type TileKind = 'start' | 'street' | 'depot' | 'works' | 'tax' | 'fortune' | 'ledger' | 'holding' | 'dispatch' | 'plaza';
...
export declare function isOwnable(tile: Tile): tile is OwnableTile;
...
export type Phase = 'lobby' | 'pre_roll' | 'awaiting_buy' | 'auction' | 'debt' | 'post_roll' | 'game_over';
...
export interface ClientToServerEvents {
    ...
    'room:leave': () => void;
    'room:action': (action: GameAction, ack?: (res: {
        ok: boolean;
        error?: string;
    }) => void) => void;
