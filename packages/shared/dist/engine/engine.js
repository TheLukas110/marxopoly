...
import { activePlayers, canBuild, canMortgage, canSellBuilding, currentPlayer, getPlayer, groupOf, liquidValue, mortgageValue, netWorth, nextTileOfKind, ownableTile, ownedTileIds, rentFor, unmortgageCost, } from './selectors.js';
const MAX_LOG = 400;
// ---------------------------------------------------------------------------
// Entry point
// ---------------------------------------------------------------------------
...
export function applyAction(state, envelope) {
    ...
    if (g.log.length > MAX_LOG)
        g.log = g.log.slice(-MAX_LOG);
    return { ok: true, state: g };
}
function dispatch(g, playerId, action, now) {
    switch (action.type) {
        case 'set_connected':
...
// Logging & money
// ---------------------------------------------------------------------------
function log(g, kind, text, playerId) {
    g.logSeq += 1;
    const entry = { id: g.logSeq, at: Date.now(), text, kind };
    if (playerId)
...
function tname(g, tileId) {
    return g.tileNames[tileId] ?? tileAt(tileId).name;
}
function credit(g, player, amount) {
    player.cash += amount;
}
/**
 ...
 */
function charge(g, player, amount, creditorId, reason) {
    if (amount <= 0)
        return true;
...
function payOut(g, creditorId, amount) {
    if (creditorId) {
    ...
    if (g.settings.plazaPot)
        g.plazaPot += amount;
}
// ---------------------------------------------------------------------------
// Lifecycle
// ---------------------------------------------------------------------------
function setConnected(g, playerId, connected) {
...
function startGame(g, playerId, now) {
    if (g.phase !== 'lobby')
        return 'The game has already started.';
    if (g.players.length < 2)
        return 'You need at least two players.';
    ...
    if (first)
        log(g, 'system', `${first.name} goes first.`, first.id);
    return null;
}
/** Snapshot every player's net worth for the end-of-game chart. */
function recordNetWorth(g) {
    const worth = {};
    for (const p of g.players)
        worth[p.id] = p.bankrupt ? 0 : netWorth(g, p.id);
...
// ---------------------------------------------------------------------------
function doRoll(g, playerId, now) {
    if (g.phase !== 'pre_roll')
        return 'You cannot roll right now.';
    ...
    if (rig) {
        [a, b] = rig;
        player.sonToesLap = ((player.sonToesLap ?? 0) + 1);
    }
        ...
        if (player.position + a + b >= SONTOES_STREETS[player.sonToesLap]) {
            player.sonToesLap = (player.sonToesLap + 1);
        }
    }
    g.dice = [a, b];
    const total = a + b;
...
const SONTOES_STREETS = [37, 39];
function movePlayerBy(g, player, steps, now) {
 ...
 */
function riggedSonToesDice(player) {
...
function moveTo(g, player, target, collectStart, now, diceTotalOverride) {
...
function diceTotal(g) {
    return g.dice ? g.dice[0] + g.dice[1] : 0;
}
function sendToHolding(g, player) {
...
// Landing resolution
// ---------------------------------------------------------------------------
function resolveLanding(g, player, total, now, rentMultiplier) {
...
function drawCard(g, player, deck, now) {
    ...
    if (card.effect.kind !== 'reprieve')
        pile.push(cardId);
    applyCard(g, player, card, now);
}
function applyCard(g, player, card, now) {
    const effect = card.effect;
    switch (effect.kind) {
        case 'cash': {
            if (effect.amount >= 0)
                credit(g, player, effect.amount);
            else
                charge(g, player, -effect.amount, null, 'a fortune card');
            return;
        }
        case 'collect_each': {
        ...
        case 'advance_nearest': {
            ...
            if (deed.ownerId === player.id || deed.mortgaged)
                return;
            const owner = getPlayer(g, deed.ownerId);
            const base = effect.target === 'works'
...
// Buying and auctions
// ---------------------------------------------------------------------------
function doBuy(g, playerId, now) {
    if (g.phase !== 'awaiting_buy')
...
function doDecline(g, playerId, now) {
    if (g.phase !== 'awaiting_buy')
...
function beginAuction(g, tileId, starterId, now) {
    ...
    const ordered = [...eligible].sort((a, b) => ((a.seat - startSeat + 8) % 8) - ((b.seat - startSeat + 8) % 8));
    g.auction = {
        tileId,
        highBid: 0,
        highBidderId: null,
...
function doBid(g, playerId, amount, now) {
    const auction = g.auction;
    if (g.phase !== 'auction' || !auction)
        return 'No auction is running.';
    if (auction.activeIds[auction.turnIndex] !== playerId)
        return 'It is not your bid.';
    const player = getPlayer(g, playerId);
    if (!player)
        return 'Unknown player.';
    const bid = Math.round(amount);
    if (!Number.isFinite(bid) || bid <= auction.highBid)
        return 'Bid must beat the current bid.';
    if (bid > player.cash)
        return 'You cannot bid more cash than you hold.';
    auction.highBid = bid;
...
function doPassBid(g, playerId, now) {
    const auction = g.auction;
    if (g.phase !== 'auction' || !auction)
        return 'No auction is running.';
    if (auction.activeIds[auction.turnIndex] !== playerId)
    ...
    if (auction.turnIndex >= auction.activeIds.length)
        auction.turnIndex = 0;
    if (player)
        log(g, 'system', `${player.name} passed.`, playerId);
    advanceAuction(g, now, true);
    return null;
}
function advanceAuction(g, now, afterPass = false) {
    const auction = g.auction;
    const done = auction.activeIds.length === 0 ||
...
// Property management
// ---------------------------------------------------------------------------
function managementAllowed(g) {
    return g.phase === 'pre_roll' || g.phase === 'post_roll' || g.phase === 'debt' || g.phase === 'awaiting_buy';
}
function doBuild(g, playerId, tileId, now) {
    if (!managementAllowed(g))
        return 'You cannot build right now.';
    if (g.phase === 'debt' && g.debt?.debtorId !== playerId)
        return 'Settle the outstanding debt first.';
    const check = canBuild(g, playerId, tileId);
    if (!check.ok)
        return check.reason ?? 'Cannot build here.';
    const player = getPlayer(g, playerId);
    const deed = g.deeds[tileId];
    player.cash -= check.cost;
    deed.houses += 1;
    const tile = tileAt(tileId);
    log(g, 'build', deed.houses === 5
        ? `${player.name} opened a hotel on ${tname(g, tile.id)}.`
        : `${player.name} built a house on ${tname(g, tile.id)} (${deed.houses}).`, playerId);
    return null;
}
function doSellBuilding(g, playerId, tileId, now) {
    if (!managementAllowed(g))
        return 'You cannot sell right now.';
    const check = canSellBuilding(g, playerId, tileId);
    if (!check.ok)
        return check.reason ?? 'Cannot sell here.';
    const player = getPlayer(g, playerId);
    const deed = g.deeds[tileId];
    deed.houses -= 1;
    player.cash += check.cost;
...
function doMortgage(g, playerId, tileId, now) {
    if (!managementAllowed(g))
        return 'You cannot mortgage right now.';
    const check = canMortgage(g, playerId, tileId);
    if (!check.ok)
        return check.reason ?? 'Cannot mortgage.';
    const player = getPlayer(g, playerId);
...
function doUnmortgage(g, playerId, tileId, now) {
    if (!managementAllowed(g))
        return 'You cannot lift a mortgage right now.';
...
// Holding yard
// ---------------------------------------------------------------------------
function doPayFine(g, playerId, now) {
    if (g.phase !== 'pre_roll')
        return 'You can only do this at the start of your turn.';
    const player = currentPlayer(g);
    if (!player || player.id !== playerId)
        return 'It is not your turn.';
    if (!player.inHolding)
        return 'You are not in the holding yard.';
    if (player.cash < g.settings.holdingFine)
        return 'Not enough cash for the fine.';
...
function doUseReprieve(g, playerId, now) {
    if (g.phase !== 'pre_roll')
        return 'You can only do this at the start of your turn.';
    const player = currentPlayer(g);
    if (!player || player.id !== playerId)
    ...
    if (!player.inHolding)
        return 'You are not in the holding yard.';
    if (player.reprieveCards <= 0)
        return 'You have no reprieve card.';
    player.reprieveCards -= 1;
    player.inHolding = false;
    player.holdingTurns = 0;
...
function validateSide(g, ownerId, side) {
    const owner = getPlayer(g, ownerId);
    if (!owner)
        return 'Unknown player.';
    if (owner.cash < side.cash)
        return `${owner.name} does not have that much cash.`;
    if (owner.reprieveCards < side.reprieveCards)
        return `${owner.name} does not have that many reprieve cards.`;
    for (const id of side.tileIds) {
        const deed = g.deeds[id];
        if (!deed || deed.ownerId !== ownerId)
            return `${owner.name} does not own ${tname(g, id)}.`;
        if (deed.houses > 0)
            return `Sell the buildings on ${tname(g, id)} before trading it.`;
...
function mortgageInterestOwed(g, tileIds) {
    let total = 0;
    for (const id of tileIds) {
        const deed = g.deeds[id];
        const tile = ownableTile(id);
        if (deed?.mortgaged && tile)
            total += Math.ceil(mortgageValue(tile) * 0.1);
    }
    return total;
}
function doProposeTrade(g, playerId, action, now) {
    if (g.phase === 'lobby' || g.phase === 'game_over')
        return 'Trading is closed.';
    if (playerId === action.toId)
    ...
    if (g.trades.filter((t) => t.fromId === playerId).length >= 5)
        return 'Too many open offers.';
    const give = sanitizeSide(action.give);
    const receive = sanitizeSide(action.receive);
    if (give.cash === 0 && give.tileIds.length === 0 && give.reprieveCards === 0 &&
    ...
    if (action.message)
        offer.message = action.message.slice(0, 200);
    g.trades.push(offer);
    log(g, 'trade', `${from.name} sent ${to.name} a trade offer.`, playerId);
    return null;
}
function doAcceptTrade(g, playerId, tradeId, now) {
    if (g.phase === 'lobby' || g.phase === 'game_over')
        return 'Trading is closed.';
    ...
    if (offer.toId !== playerId)
        return 'This offer is not addressed to you.';
    ...
    for (const id of offer.give.tileIds)
        g.deeds[id].ownerId = to.id;
    for (const id of offer.receive.tileIds)
        g.deeds[id].ownerId = from.id;
    if (fromInterest > 0) {
    ...
    g.trades = g.trades.filter((t) => ![...t.give.tileIds, ...t.receive.tileIds].some((id) => touched.has(id)));
    log(g, 'trade', `${from.name} and ${to.name} agreed a trade.`, playerId);
    maybeSettleDebt(g, now);
    return null;
}
function doDropTrade(g, playerId, tradeId, _now) {
    const index = g.trades.findIndex((t) => t.id === tradeId);
    if (index === -1)
        return 'That offer is gone.';
    const offer = g.trades[index];
    if (offer.fromId !== playerId && offer.toId !== playerId)
        return 'Not your offer.';
    g.trades.splice(index, 1);
    const actor = getPlayer(g, playerId);
    if (actor)
        log(g, 'trade', `${actor.name} ${offer.fromId === playerId ? 'withdrew' : 'declined'} a trade offer.`, playerId);
    return null;
}
// ---------------------------------------------------------------------------
// Debt, bankruptcy, turn flow
// ---------------------------------------------------------------------------
function maybeSettleDebt(g, now) {
    const debt = g.debt;
    if (!debt)
        return;
    ...
    log(g, 'money', `${debtor.name} settled ${money(debt.amount)}${creditor ? ` with ${creditor.name}` : ' with the bank'}.`, debtor.id);
    g.debt = null;
    settleTurn(g, now);
}
function doDeclareBankruptcy(g, playerId, now) {
    if (g.phase !== 'debt' || !g.debt)
        return 'You have no outstanding debt.';
    if (g.debt.debtorId !== playerId)
        return 'You are not the debtor.';
 ...
 */
function doResign(g, playerId, now, reason) {
    const player = getPlayer(g, playerId);
    if (!player || player.bankrupt)
        return 'You are already out.';
    if (g.phase === 'lobby' || g.phase === 'game_over')
        return 'Nothing to resign from.';
    log(g, 'system', reason === 'bankrupt'
        ? `${player.name} reported bankrupt and is out of the game.`
        : `${player.name} left the table.`, playerId);
    bankrupt(g, playerId, null, now);
    return null;
}
function bankrupt(g, debtorId, creditorId, now) {
    ...
    if (creditor) {
        ...
        log(g, 'system', `${debtor.name} went bankrupt. ${creditor.name} takes everything they had.`, debtorId);
    }
    else {
        for (const id of tiles) {
    ...
    if (g.debt?.debtorId === debtorId)
        g.debt = null;
    if (g.auction) {
        g.auction.activeIds = g.auction.activeIds.filter((id) => id !== debtorId);
        if (g.auction.turnIndex >= g.auction.activeIds.length)
            g.auction.turnIndex = 0;
    }
    ...
    if (remaining.length <= 1) {
        ...
        recordNetWorth(g);
        if (remaining[0])
            log(g, 'system', `${remaining[0].name} wins with ${money(netWorth(g, remaining[0].id))} in assets.`);
        return;
    }
    if (g.auction) {
        advanceAuction(g, now, true);
 ...
 */
function settleTurn(g, now) {
    if (g.phase === 'game_over' || g.phase === 'lobby')
        return;
    if (g.debt) {
        g.phase = 'debt';
        return;
...
function doEndTurn(g, playerId, now) {
...
function advanceTurn(g, now) {
    ...
    const player = currentPlayer(g);
    if (player)
        log(g, 'system', `It is ${player.name}'s turn.`, player.id);
}
// ---------------------------------------------------------------------------
// Timeouts (server-driven)
// ---------------------------------------------------------------------------
function doTimeout(g, now) {
    switch (g.phase) {
        case 'pre_roll': {
            const player = currentPlayer(g);
            if (!player)
        ...
        case 'debt': {
            ...
            if (g.debt)
                bankrupt(g, debt.debtorId, debt.creditorId, now);
            return null;
        }
        default:
            return null;
...
export function autoLiquidate(g, playerId, now) {
    const debt = g.debt;
